...
# Conclusion
1. Both hypothesis 1 and hypothesis 2 are partially supported by three different models, linear, general additive, and random forest. They substantiate the positive effects of positive emotion and negative effect of negative emotion on the general evaluation score.
2. For specific emotions, trust, joy, sadness, and fear have the most significant influence on the general evaluation score among all emotions.
3. These models are robust and produce stable output regarding the k-fold cross validation and training-testing sample fitting.
4. The order of general effectiveness evaluated by model MSE is, from high to low, linear model, general additive model, and random forest. Linear model is not necessarily worse, compared to those complicated ones like GAM and random forset.
5. However, through only emotions, all three models provide only limited explanation regarding the variance of general evaluation score.
...
# Bonus
## App of Game Experiential Topic Comparison
